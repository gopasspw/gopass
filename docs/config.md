...
| `age.ssh-key-path`              | `string` | Path to a custom SSH key file or directory containing SSH keys. A leading `~/` is expanded to the home directory. If set, this will be used to load SSH identities for age. If not set, the default SSH directory is used.                                                           | `~/.ssh`                            |
