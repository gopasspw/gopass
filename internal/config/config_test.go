...
func TestNewConfig(t *testing.T) {
	...
	cfg := config.New()
	cs := cfg.String()
	assert.Contains(t, cs, `&config.Config{AutoClip:false, AutoImport:true, ClipTimeout:45, ConfirmRecipients:false, EditRecipients:false, ExportKeys:true, NoColor:false, NoPager:false, Notifications:true,`)
	...
	cfg.Mounts["bar"] = ""
	cs = cfg.String()
	assert.Contains(t, cs, `&config.Config{AutoClip:false, AutoImport:false, ClipTimeout:0, ConfirmRecipients:false, EditRecipients:false, ExportKeys:false, NoColor:false, NoPager:false, Notifications:false,`)
