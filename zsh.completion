...
_gopass () {
	...
	case "${cmd}" in
	  ...
	  move|mv)
	      ...
	      ;;
	  otp|totp|hotp)
	      _arguments : "--clip[Copy the time-based token into the clipboard]" "--qr[Write QR code to FILE]" "--chained[chain the token to the password]" "--password[Only display the token]" "--snip[Scan screen content to insert a OTP QR code into provided entry]"
